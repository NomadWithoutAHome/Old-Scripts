...
local event = game:GetService("ReplicatedStorage").poopydiaper
...
Home:addLabel("Nerf FPS GUI v2.3")
Home:addLabel("")
...
Home:addLabel("yyesok - Celestial Silent Aim")
...
SilentAim:addLabel('Silent Aim Options', 'various options for silent aim')
SilentAim:addLabel('')
SilentAim:addDropdown('Select Body Part', {'Head', 'Torso', 'HumanoidRootPart'}, 1, function(value) getgenv().SelectedPart = value print(getgenv().SelectedPart) end)
SilentAim:addSlider("FOV",0,400,function(value) getgenv().FOV = tonumber(value) end)
SilentAim:addSlider("Distance",0,1000,function(value) getgenv().Distance = tonumber(value) end)
SilentAim:addToggle('CircleVisibility', function(bool) getgenv().CircleVisibility = bool end)
SilentAim:addButton('Silent Aim', function() loadstring(game:HttpGet("https://raw.githubusercontent.com/yesok3877/Celestial-Silent-Aim/master/Auto-Input", true))() end)
...
function playerJoined(player)
    for _, v in pairs(gameMods) do
        if player.UserId == v then
            sendChat('----------WARNING----------')
            sendChat(player.Name.. ' Has Joined!')
            sendChat('They are a known Admin.')
            sendChat('Please be Advised!')
            sendChat('----------WARNING----------')
        elseif player.UserId == 2221809093 then 
            sendChat('The Creator of the script has joined your game!')
        end
    end
end
...
function playerLeft(player)
    for _, v in pairs(gameMods) do
        if player.UserId == v then
            sendChat('The Admin '.. player.Name.. ' Has Left!')
        end
    end
end
